  ...
  group('FirebaseAuthRepository', () {
    ...
    group('user', () {
      test('return user stream and convert UserRecord to User', () async {
        when(firebaseAuth.userChanges()).thenAnswer((_) => Stream.fromIterable([mockUser, null]));
    ...
    group('registerWithEmailAndPassword', () {
      test('register user with email and password, and return User', () async {
        when(firebaseAuth.createUserWithEmailAndPassword(email: 'test_email', password: 'test_password'))
            .thenAnswer(
      ...
      test('throws an exception if call completes with an error', () async {
        var throwable = firebase_auth.FirebaseAuthException(code: 'error');
        when(firebaseAuth.createUserWithEmailAndPassword(email: 'test_email', password: 'test_password')).thenThrow(
          throwable,
        );
    ...
    group('loginWithEmailAndPassword', () {
      test('login user with email and password', () async {
        when(firebaseAuth.signInWithEmailAndPassword(email: 'test_email', password: 'test_password')).thenAnswer(
          (_) async => mockUserCredential,
        );
      ...
      test('throws an exception if call completes with an error', () async {
        var throwable = firebase_auth.FirebaseAuthException(code: 'error');
        when(firebaseAuth.signInWithEmailAndPassword(email: 'test_email', password: 'test_password')).thenThrow(
          throwable,
        );
    ...
    group('logout', () {
      ...
      test('throws an exception if logout completes with an error', () async {
        var throwable = firebase_auth.FirebaseAuthException(code: 'error');
        when(firebaseAuth.signOut()).thenThrow(throwable);
